...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    gradW,grads['W2'],grads['b2']=affine_backward(gradW, aff2_cache)
    gradW,grads['W1'],grads['b1']=affine_relu_backward(gradW,aff_relu_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(len(hidden_dims)+1):
      input_dim_createW= input_dim if i==0 else hidden_dims[i-1]
      if(i!=len(hidden_dims)):
        self.params['W'+str(i+1)]=np.random.normal(0,weight_scale,(input_dim_createW,hidden_dims[i]))
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers-1):
      ...
      if not self.use_batchnorm:
        ...
        #affine_relu_batchnormalize
        current_input, layer_cache[keya] = affine_bn_relu_forward(current_input, 
          self.params[keya], self.params[keyb],
          self.params[keygamma],self.params[keybeta],
          self.bn_params[i] )
    ...
    #last layer affine
    scores, layer_cache['W'+str(self.num_layers)] = affine_forward(current_input,
    ...
    keya = 'W'+str(i+1)
    keyb = 'b'+str(i+1)
    dx,grads[ keya ],grads[ keyb ]=affine_backward(dx, layer_cache[ keya ])
    ...
    for i in reversed(range(self.num_layers-1)):
      ...
      keyb = 'b'+str(i+1)
      if not self.use_batchnorm: 
        dx,grads[ keya ],grads[ keyb ]=affine_relu_backward(dx, layer_cache[ keya ])
        grads[ keya ]+=self.params[ keya ]*self.reg
      else:
        keygamma='gamma'+str(i+1)
        keybeta='beta'+str(i+1)
        dx,grads[ keya ],grads[ keyb ], dgamma, dbeta = affine_bn_relu_backward(dx, layer_cache[ keya ]) 
        grads[ keya ]+=self.params[ keya ]*self.reg
        grads[ keybeta ]=dbeta
