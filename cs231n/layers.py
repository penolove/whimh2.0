...
def batchnorm_backward(dout, cache):
  ...
  #dsample_var = sum( dl/dx' * dx'/dvar ,axis=0)

  dsample_var= np.sum( dx_normalized * (-0.5*(x-sample_mean)/((sample_var+eps)**(3.0/2))) ,axis =0)


  #gradient from x'
  dsample_mean_xp=-np.sum(dx_normalized/np.sqrt(sample_var+eps),axis =0)
  #gradient from Var
  dsample_mean_Var=-2.0/N*dsample_var*np.sum((x-sample_mean),axis =0)
  #dsample_var=dsample_mean_xp+dsample_mean_Var
  dsample_mean=dsample_mean_xp+dsample_mean_Var


  #three gradient source
...
def batchnorm_backward_alt(dout, cache):
  ...
  #no consider graident from var to e(x)
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    mask=(np.random.uniform(low=0.0,high=1.0,size=x.shape)<(1-p))/(1-p)
    out=x*mask
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
  ...
  for n in range(N):
    for i in range(H_out):
      for j in range(W_out):
        for f in range(F):
          current_matrix = x_pad[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW]
          current_filter = w[f]
          out[n,f,i,j] = np.sum(current_matrix*current_filter)
...
def conv_backward_naive(dout, cache):
  ...
  for n in range(N):
    for f in range(F):
      for i in range(H_out):
        for j in range(W_out):
          courrent_x_matrix = x_pad[n,:,i*stride:i*stride+HH,j*stride:j*stride+WW]
          dw[f]=dw[f]+dout[n,f,i,j]*courrent_x_matrix
    ...
    for c in range(C):
      for h in range(H_out):
        for w in range(W_out):
          ...
          idx=np.argmax(current_matrix)
          h_pool=idx/pool_width
          w_pool=idx%pool_width
          dx[n,c,h*stride+h_pool,w*stride+w_pool]+=dout[n,c,h,w]
  #############################################################################
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
