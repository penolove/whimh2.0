...
def affine_bn_relu_forward(x, w, b,gamma, beta, bn_param):
  ...
  """
  a, fc_cache = affine_forward(x, w, b)
  bn_norm, bn_cache =batchnorm_forward(a, gamma, beta, bn_param)
  out, relu_cache = relu_forward(bn_norm)
...
def affine_bn_relu_backward(dout, cache):
  ...
  da = relu_backward(dout, relu_cache)
  bndx, dgamma, dbeta = batchnorm_backward_alt(da, bn_cache)
  dx, dw, db = affine_backward(bndx, fc_cache)
...
def conv_relu_bn_forward(x, w, b, conv_param, gamma, beta, bn_param):
  """
  Convenience layer that performs a convolution, a ReLU, and a pool.
  ...
  cache = (conv_cache, relu_cache, bn_cache)
  return out, cache


def conv_relu_bn_backward(dout, cache):
  """
  Backward pass for the conv-relu-pool convenience layer
  """
  conv_cache, relu_cache, bn_cache = cache
  dbn, dgamma, dbeta = spatial_batchnorm_backward(dout, bn_cache)
...
def conv_relu_bn_pool_forward(x, w, b, conv_param, pool_param, gamma, beta, bn_param):
  """
  ...
  a, conv_cache = conv_forward_fast(x, w, b, conv_param)
  s, relu_cache = relu_forward(a)
  bn, bn_cache = spatial_batchnorm_forward(s, gamma,beta, bn_param)
  out, pool_cache = max_pool_forward_fast(bn, pool_param)
...
def conv_relu_bn_pool_backward(dout, cache):
  ...
  ds = max_pool_backward_fast(dout, pool_cache)
  dbn, dgamma, dbeta = spatial_batchnorm_backward(ds, bn_cache)
  da = relu_backward(dbn, relu_cache)
  dx, dw, db = conv_backward_fast(da, conv_cache)
  return dx, dw, db ,dgamma ,dbeta
